...
#                  also set by FWTH_VERBOSE=1. Budgets are still enforced.
#
...
#
# The same script as fs-linux-test-harness's, so a consumer of either harness
# budgets its tiers the same way; only the environment variable differs,
# because each harness reads its own prefix.
#
...
if [ -n "$over" ]; then
    ...
    echo "             harness README, \"Output: quiet by default\"." >&2
